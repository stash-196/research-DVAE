...
class Options:
    ...
    def _initial(self):
        ...
        self.parser.add_argument(
            '--ss', action='store_true', help='schedule sampling')
        self.parser.add_argument(
            '--cfg', type=str, default=None, help='config path')
        self.parser.add_argument(
            '--device_cfg', type=str, default='config/cfg_device.ini', help='device config path')
        self.parser.add_argument(
    ...
    def get_params(self):
        ...
        if params['cfg'] is None:
            params['cfg'] = os.path.join(os.path.dirname(
                params['saved_dict']), 'config.ini')
...
if __name__ == '__main__':
    ...
    merged_config_path = os.path.join(os.path.dirname(
        params['saved_dict']), 'merged_config.ini')
    ...
    print("[Eval] Total params: %.2fM" % (sum(p.numel()
          for p in dvae.parameters()) / 1000000.0))
    ...
    if cfg['DataFrame']["dataset_name"] == "Sinusoid":
        # Load test dataset
        test_dataset = sinusoid_dataset.Sinusoid(path_to_data=data_dir, split='test', seq_len=seq_len, x_dim=x_dim, sample_rate=sample_rate,
                                                 observation_process=observation_process, device=device, overlap=overlap, skip_rate=skip_rate, val_indices=1, shuffle=False, s_dim=s_dim)
        # Build test dataloader
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=len(
            test_dataset), shuffle=False, num_workers=num_workers)
        # Load test dataset for long sequence
        test_dataset_long = sinusoid_dataset.Sinusoid(path_to_data=data_dir, split='test', seq_len=None, x_dim=x_dim, sample_rate=sample_rate,
                                                      observation_process=observation_process, device=device, overlap=overlap, skip_rate=skip_rate, val_indices=1, shuffle=False, s_dim=s_dim)
        # Build test dataloader
        ...
        # Load test dataset
        test_dataset = lorenz63_dataset.Lorenz63(path_to_data=data_dir, split='test', seq_len=seq_len, x_dim=x_dim, sample_rate=sample_rate,
                                                 observation_process=observation_process, device=device, overlap=overlap, skip_rate=skip_rate, val_indices=1, shuffle=False)
        # Build test dataloader
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=len(
            test_dataset), shuffle=False, num_workers=num_workers)

        # Load test dataset for long sequence
        test_dataset_long = lorenz63_dataset.Lorenz63(path_to_data=data_dir, split='test', seq_len=None, x_dim=x_dim, sample_rate=sample_rate,
                                                      observation_process=observation_process, device=device, overlap=overlap, skip_rate=skip_rate, val_indices=1, shuffle=False)
        # Build test dataloader
        test_dataloader_long = torch.utils.data.DataLoader(
            test_dataset_long, batch_size=1, shuffle=False, num_workers=num_workers)
    ...
    def calculate_expected_accuracy(true_data, reconstructed_data, accuracy_measure):
        ...
        reshaped_true = true_data.reshape(
    ...
    def calculate_power_spectrum_error(power_spectrum_lst, true_signal_index, filter_std):
        """
        ...
        for power_spectrum in power_spectrum_lst:
            power_spectrum_errors.append(power_spectrum_error(
    ...
    with torch.no_grad():
        ...
        # Visualize results
        save_fig_dir = os.path.join(os.path.dirname(
            params['saved_dict']), 'post_training_figs')
        ...
        for i, batch_data_long in enumerate(test_dataloader_long):
            full_xyz_data = test_dataloader_long.dataset.get_full_xyz(
                i)  # Get full xyz data for the same index
            ...
            batch_data_long_tensor = torch.tensor(
                batch_data_long, device=dvae.device)
            recon_data_long = dvae(
                batch_data_long_tensor, mode_selector=autonomous_mode_selector_long, inference_mode=True).cpu().numpy()

            # Visualize the spectral analysis
            long_data_lst = [full_xyz_data[:, 1], full_xyz_data[:, 2], batch_data_long[:, 0, :].reshape(
                -1), recon_data_long[~autonomous_mode_selector_long, 0, :].reshape(-1), recon_data_long[autonomous_mode_selector_long, 0, :].reshape(-1)]
            ...
            power_spectrum_lst, frequencies, periods = visualize_spectral_analysis(
                data_lst=long_data_lst, name_lst=name_lst, colors_lst=colors_lst, save_dir=save_fig_dir, sampling_rate=sampling_rate, max_sequences=50000)

            power_spectrum_error_lst = calculate_power_spectrum_error(
                power_spectrum_lst, true_signal_index, filter_std=3)
            visualize_errors_from_lst(power_spectrum_error_lst, name_lst=name_lst, save_dir=save_fig_dir,
                                      explain='power_spectrum_error', error_unit='dB', colors=colors_lst)
            ...
            if learning_algo.optimize_alphas is not None:
                ...
                if cfg['DataFrame']["dataset_name"] == "Lorenz63":
                    ...
                    if s_dim == 1:
                        ...
                        true_alphas = [dt*2*np.pi, dt *
                                       2*np.pi/100, 2*np.pi/1000]
                else:
                    raise ValueError(
                        "Unsupported dataset_name in configuration file.")
                visualize_alpha_history_and_spectrums(sigmas_history=sigmas_history, power_spectrum_lst=power_spectrum_lst[:3], spectrum_color_lst=colors_lst[
                                                      :3], spectrum_name_lst=name_lst, frequencies=frequencies, dt=dt, save_dir=save_fig_dir, kl_warm_epochs=kl_warm_epochs, true_alphas=true_alphas)

            # Plot the reconstruction vs true sequence
            visualize_teacherforcing_2_autonomous(batch_data_long, dvae, mode_selector=autonomous_mode_selector_long,
                                                  save_path=save_fig_dir, explain='final_long_inference_mode', inference_mode=True)
            visualize_teacherforcing_2_autonomous(batch_data_long, dvae, mode_selector=autonomous_mode_selector_long,
                                                  save_path=save_fig_dir, explain='final_long_generative_mode', inference_mode=False)
            ...
            if False:
                ...
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'], technique='kernel_pca')
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'], technique='isomap')
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'], technique='lle')
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'], technique='umap')
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'], technique='ica')
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'], technique='mds')
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'])
                visualize_embedding_space([teacherforced_states, autonomous_states], save_dir=save_fig_dir, variable_name='hidden', condition_names=[
                                          f'teacher-forced', f'autonomous'], base_colors=['Greens', 'Reds'], technique='tsne')
        ...
        recon_data_repeated = dvae(
            batch_data_repeated_tensor, mode_selector=autonomous_mode_selector).cpu().numpy()
        ...
        visualize_accuracy_over_time(expected_rmse, expected_rmse_variance, save_dir, measure='rsme', num_batches=batch_size,
                                     num_iter=num_iterations, explain="over multiple series", autonomous_mode_selector=expanded_autonomous_mode_selector)
        visualize_accuracy_over_time(expected_r2, expected_r2_variance, save_dir, measure='r2', num_batches=batch_size,
                                     num_iter=num_iterations, explain="over multiple series", autonomous_mode_selector=expanded_autonomous_mode_selector)

        # visualize the hidden states
        visualize_variable_evolution(dvae.h, batch_data=batch_data, save_dir=save_fig_dir,
                                     variable_name=f'hidden', alphas=alphas_per_unit, add_lines_lst=[half_point])

        # visualize the x_features
        visualize_variable_evolution(dvae.feature_x, batch_data=batch_data,
                                     save_dir=save_fig_dir, variable_name=f'x_features', add_lines_lst=[half_point])

        # Check if the model has a z variable
        if hasattr(dvae, 'z_mean'):
            # visualize the latent states
            visualize_variable_evolution(dvae.z_mean, batch_data=batch_data, save_dir=save_fig_dir,
                                         variable_name=f'z_mean_posterior', add_lines_lst=[half_point])
            visualize_variable_evolution(dvae.z_logvar, batch_data=batch_data, save_dir=save_fig_dir,
                                         variable_name=f'z_logvar_posterior', add_lines_lst=[half_point])
            visualize_variable_evolution(dvae.z_mean_p, batch_data=batch_data, save_dir=save_fig_dir,
                                         variable_name=f'z_mean_prior', add_lines_lst=[half_point])
            visualize_variable_evolution(dvae.z_logvar_p, batch_data=batch_data, save_dir=save_fig_dir,
                                         variable_name=f'z_logvar_prior', add_lines_lst=[half_point])

        # Plot the reconstruction vs true sequence
        visualize_teacherforcing_2_autonomous(batch_data, dvae, mode_selector=autonomous_mode_selector,
                                              save_path=save_fig_dir, explain='final_generative_mode', inference_mode=False)
        visualize_teacherforcing_2_autonomous(batch_data, dvae, mode_selector=autonomous_mode_selector,
                                              save_path=save_fig_dir, explain='final_inference_mode', inference_mode=True)
