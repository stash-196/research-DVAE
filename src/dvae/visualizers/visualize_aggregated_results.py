...
def visualize_aggregated_metrics(aggregated_metrics, save_dir):
    ...
    for key_tuple, metrics in aggregated_metrics.items():
        ...
        grouped_metrics[primary_value][secondary_value][tuple(
            residual_values)].append((key_tuple, metrics))

    unique_primary_values = sorted(grouped_metrics.keys())
    primary_x_labels = generate_primary_x_labels(
        param_names[0], unique_primary_values)
    ...
    for primary_value, secondary_dict in grouped_metrics.items():
        for residual_values, sub_dict in secondary_dict.items():
            ...
            if residual_values:
                residual_str = ", ".join(
                    [f"{param_names[i+2]}={v}" for i, v in enumerate(residual_values) if i+2 < len(param_names)])
            ...
            for j, (secondary_value, entries) in enumerate(sub_dict.items()):
                ...
                ax.bar(index + j * bar_width, mean_values[:len(index)], bar_width, yerr=std_values[:len(
                    index)], label=f'{param_names[1]}={secondary_value}')
            ...
            residual_str = "_".join([f"{param_names[i+2]}={v}" for i, v in enumerate(
                residual_values) if i+2 < len(param_names)]) if residual_values else ""
            plt.savefig(os.path.join(
                save_dir, f'metrics_{primary_value}_{residual_str}.png'))
            plt.close()
            print(
                f"Saved plot for {param_names[0]} = {primary_value}, {residual_str}")
