...
class DampedSimpleHarmonicOscillator(Dataset):
    ...
    def apply_observation_process(self, sequence) -> torch.Tensor:
        ...
        if self.observation_process == "mixed_1d":
            ...
            print(
                f"[DampedSHO][only_x_interpolate] NaNs before: {n_nan_before}, after: {n_nan_after}"
            )
            sequence = x
            ...
            print(
                f"[DampedSHO][only_x_indicate] missing_count: {missing_count}, result_shape: {result.shape}"
            )
            sequence = result
        else:
