...
for experiment in "${experiments[@]}"; do
    ...
    find "$CONFIG_DIR" -name "*.ini" | while read CONFIG_FILE; do
        ...
        cat > "temp/run_training_$CONFIG_BASENAME.slurm" <<EOL
#!/bin/bash
...
# Run the command
pyenv-research-DVAE/bin/python train_model.py --cfg $CONFIG_FILE --job_id \$SLURM_JOBID
...
EOL
        ...
        sbatch "temp/run_training_$CONFIG_BASENAME.slurm"
