...
def build_dataloader(cfg):
    ...
    # Load dataset
    train_dataset = Lorenz63(path_to_data=data_dir, split=0, seq_len=sequence_len, sample_rate=sample_rate, skip_rate=skip_rate, val_indices=val_indices)
    val_dataset = Lorenz63(path_to_data=data_dir, split=2, seq_len=sequence_len, sample_rate=sample_rate, skip_rate=skip_rate, val_indices=val_indices)
    test_dataset = Lorenz63(path_to_data=data_dir, split=1, seq_len=sequence_len, sample_rate=sample_rate, skip_rate=skip_rate, val_indices=val_indices)
...
class Lorenz63(Dataset):
    def __init__(self, path_to_data, split, seq_len, sample_rate, skip_rate, val_indices):
        """
        ...
        self.seq = torch.from_numpy(the_sequence).float().cuda()
