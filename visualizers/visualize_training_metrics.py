...
def visualize_combined_metrics(delta_per_epoch, kl_warm_epochs, auto_warm_epochs, kl_warm_values, cpt_patience_epochs, best_state_epochs, model_name, save_figures_dir, tag):
    ...
    axs[1].step(kl_warm_epochs, kl_warm_values, label='kl_warm')
    ...
    axs[3].step(range(len(best_state_epochs)), best_state_epochs, label='best_state')
    ...
    fig_file = os.path.join(save_figures_dir, f'vis_training_delta_kl_cpt_best_state_{tag}.png')
    plt.savefig(fig_file)
    plt.close(fig)
...
def visualize_sigma_history(sigmas_history, kl_warm_epochs, auto_warm_epochs, model_name, save_figures_dir, tag):
    ...
    fig_file = os.path.join(save_figures_dir, f'vis_training_history_of_sigma_{tag}.png')
    plt.savefig(fig_file)
    plt.close(fig)
...
def visualize_alpha_history(sigmas_history, kl_warm_epochs, auto_warm_epochs, model_name, save_figures_dir, tag):
    ...
    plt.yscale('log')
    fig_file = os.path.join(save_figures_dir, f'vis_training_history_of_alpha_{tag}.png')
    plt.savefig(fig_file)
    plt.close(fig)
